import os, sys, zipfile, shutil, uuid, re, threading, subprocess, json, gc
import fitz
import pandas as pd
...
IGNORE_FOLDERS = {"payment screenshots","payments","misc","receipts",
                  "__macosx",".ds_store","payment"}
jobs = {}

# ── OCR worker script written to disk once at startup ─────────────────────────
WORKER_PATH = "/tmp/ocr_worker.py"
WORKER_CODE = r'''
import sys, cv2, pytesseract, re, json, gc
import numpy as np
...
.nav-logo-box{width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,var(--gold),#8b6914);
  display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 0 20px var(--goldglow);}
.nav-name{font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:500;color:#fff;}
.nav-name span{color:var(--gold)}
.nav-pills{display:flex;gap:8px;}
.nav-pill{font-size:11px;letter-spacing:1.5px;text-transform:uppercase;padding:5px 14px;
  border-radius:999px;color:var(--silver);border:1px solid var(--border);background:rgba(255,255,255,0.02);}
...
.hero-eyebrow::before,.hero-eyebrow::after{content:'';flex:1;height:1px;width:40px;
...
.panel{background:rgba(255,255,255,0.025);border:1px solid var(--border);border-radius:var(--r);
  backdrop-filter:blur(20px);position:relative;overflow:hidden;margin-bottom:20px;}
...
.drop-zone{border:1px dashed rgba(201,168,76,0.2);border-radius:12px;padding:32px 20px;
  text-align:center;cursor:pointer;transition:all .3s;background:rgba(201,168,76,0.02);}
...
.dz-file{margin-top:10px;font-size:12px;color:var(--green);font-family:'DM Mono',monospace;display:none;}
.dz-file.visible{display:block}
input[type="file"]{display:none}
.run-btn{width:100%;padding:18px;border:none;border-radius:12px;
  background:linear-gradient(135deg,#c9a84c,#e8cc7a 40%,#c9a84c);color:#0a0804;
...
.prog-label{font-size:13px;color:var(--gold);font-weight:500}
.prog-pct{font-family:'Cormorant Garamond',serif;font-size:28px;font-weight:300;color:#fff;}
...
.metric{background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:14px;
  padding:24px 20px;text-align:center;transition:transform .2s;position:relative;overflow:hidden;}
...
.dl-btn{width:100%;padding:18px;border:none;border-radius:12px;background:transparent;
  ...
  font-size:15px;cursor:pointer;transition:all .3s;display:flex;align-items:center;justify-content:center;gap:12px;}
.dl-btn:hover{background:rgba(46,204,143,0.08);border-color:rgba(46,204,143,0.6);transform:translateY(-2px);}
...
.footer-brand{font-family:'Cormorant Garamond',serif;font-size:13px;color:var(--muted);}
...
@media(max-width:640px){.drop-row,.steps-grid{grid-template-columns:1fr 1fr}
...
<nav>
  ...
  <div class="nav-pills"><div class="nav-pill">v4.1</div><div class="nav-pill live">System Online</div></div>
</nav>
<div class="page">
  <div class="hero fade-up">
    <div class="hero-eyebrow">Purchase Intelligence Platform</div>
    <h1>Audit Every Bill.<br><em>Instantly.</em></h1>
    <p class="hero-sub">Upload your bill images and Zoho export. Smart OCR scans every document, matches records with date and item intelligence, and delivers a verified audit report.</p>
  </div>
  <div class="panel fade-up"><div class="panel-body">
    ...
    <div class="drop-row">
      <div class="drop-zone" id="zipZone" onclick="document.getElementById('fileInput').click()">
        <div class="dz-icon">&#x1F5C2;</div><div class="dz-title">Bill Images ZIP</div>
        <div class="dz-sub">JPG &middot; PNG &middot; PDF</div>
      ...
      <div class="drop-zone" id="csvZone" onclick="document.getElementById('csvInput').click()">
        <div class="dz-icon">&#x1F4CA;</div><div class="dz-title">Zoho Excel / CSV</div>
        <div class="dz-sub">Reference for matching</div>
  ...
  <div class="panel" id="progressPanel"><div class="panel-body">
    ...
    <div class="terminal">
      <div class="terminal-bar"><div class="t-dot"></div><div class="t-dot"></div><div class="t-dot"></div>
        <span class="terminal-title">OCR Output Stream</span></div>
  ...
  <div class="panel fade-up"><div class="panel-body">
    ...
    <div class="steps-grid">
      ...
      <div class="step-card"><div class="step-num">02</div><div class="step-title">OCR Scan</div><div class="step-desc">Every bill is scanned in isolation — no memory overload, no crashes.</div></div>
      <div class="step-card"><div class="step-num">03</div><div class="step-title">Smart Match</div><div class="step-desc">Date, vendor, item and amount all scored together for reliable results.</div></div>
...
def extract_date_from_filename(filename):
    """
    Parse dates from bill image filenames:
      09_01_26.jpg       → 2026-01-09
      17_01_26.jpg       → 2026-01-17
      Image_17-Jan-2026  → 2026-01-17
    Returns ISO string or None.
    """
    name = os.path.splitext(os.path.basename(filename))[0]

    # Pattern: DD_MM_YY or DD_MM_YYYY
    m = re.search(r'(\d{2})[_\-](\d{2})[_\-](\d{2,4})', name)
    if m:
        day, month, year = m.group(1), m.group(2), m.group(3)
        if len(year) == 2:
            year = "20" + year
        # Basic sanity: day 01-31, month 01-12
        if 1 <= int(day) <= 31 and 1 <= int(month) <= 12:
            return f"{year}-{month.zfill(2)}-{day.zfill(2)}"

    # Pattern: Image_DD-Mon-YYYY  e.g. Image_17-Jan-2026
    months = {"jan":"01","feb":"02","mar":"03","apr":"04","may":"05","jun":"06",
               "jul":"07","aug":"08","sep":"09","oct":"10","nov":"11","dec":"12"}
    m2 = re.search(r'(\d{1,2})[_\-]([a-zA-Z]{3})[_\-](\d{4})', name)
    if m2:
...
def ocr_image_subprocess(img_path):
    """
    Run OCR in an isolated subprocess.
    The child process loads OpenCV + Tesseract, runs OCR, prints JSON, and exits.
    All image memory is FULLY released when the subprocess exits — no leak.
    This is the key fix for the 512MB Render memory crash.
    """
    try:
        result = subprocess.run(
            [sys.executable, WORKER_PATH, img_path],
        ...
        print(f"Subprocess OCR error {img_path}: {e}")
        return ""
...
def bill_number_score(bill_no_raw, ocr_text):
    if not bill_no_raw or len(bill_no_raw) < 2:
        return 0, False
    forms = {normalize(bill_no_raw), normalize_billno(bill_no_raw)}
    ...
    return best * 0.15, False


def score_match(bill_no, vendor, item, amount_str, ocr_text, bill_date, file_date):
    """
    Scoring breakdown (max 100):
      Bill number in OCR    → 0–45 pts
      Date match (filename) → 0–25 pts  ← NEW: compensates for garbled headers
      Vendor fuzzy          → 0–15 pts
      Item fuzzy            → 0–30 pts  ← increased weight (most reliable signal)
      Amount match          → 0–10 pts
    """
    if not ocr_text:
        return 0
    score, _ = bill_number_score(bill_no, ocr_text)

    # Date bonus: bill date from CSV vs date parsed from image filename
    if bill_date and file_date:
        # Normalise both to YYYY-MM-DD prefix (first 10 chars)
        bd = str(bill_date).strip()[:10]
        fd = str(file_date).strip()[:10]
        if bd == fd:
            score += 25

    if vendor and len(vendor) >= 2:
        score += fuzz.partial_ratio(vendor, ocr_text) * 0.15

    if item and len(item) >= 2:
        score += fuzz.partial_ratio(item, ocr_text) * 0.30
...
def run_audit(job_id, work_dir, zip_path, csv_path):
    ...
    try:
        ...
        log(f"Found {total_imgs} bill images", "info")

        # ── OCR each image in a subprocess (memory-safe) ──────────────────────
        ocr_cache = []   # list of (img_path, filename, file_date, ocr_text)
        for i, (img_path, rel_path) in enumerate(bill_images):
            fname = os.path.basename(rel_path)
            update(16 + int((i / max(total_imgs, 1)) * 50),
                   f"OCR {i+1}/{total_imgs}: {fname}")

            file_date = extract_date_from_filename(fname)
            txt = ocr_image_subprocess(img_path)

            if txt and len(txt.strip()) > 10:
                ocr_cache.append((img_path, fname, file_date, txt))
                date_info = f" date={file_date}" if file_date else ""
                log(f"OCR OK: {fname}{date_info} ({len(txt)} chars)", "ok")
            else:
                log(f"OCR empty: {fname}", "err")
        ...
        if csv_df is not None:
            ...
            log("Smart matching with date + item signals...", "info")
            ...
            for idx, (_, row) in enumerate(csv_df.iterrows()):
                ...
                bill_date = str(row.get("Bill Date", "")).strip()[:10]

                best_score = 0; best_path = ""; best_fname = ""; best_signals = ""

                for img_path, fname, file_date, text in ocr_cache:
                    s = score_match(bill_no, vendor, item, amount,
                                    text, bill_date, file_date)
                    if s > best_score:
                        best_score = s; best_path = img_path; best_fname = fname
                        ...
                        if file_date and bill_date == file_date:
                            sigs.append(f"Date:{file_date}")
                        vr = fuzz.partial_ratio(vendor, text)
                        if vr >= 55: sigs.append(f"Vendor:{vr}%")
                ...
                log(f"{status} ({best_score}) | {bill_no} | "
                    f"{str(row.get('Vendor Name',''))[:15]} | {best_signals}", t)
        else:
            for img_path, fname, file_date, _ in ocr_cache:
                results.append({
                    "file_name":fname,"folder":"","bill_number":"","bill_date":"",
        ...
        mismatch = sum(1 for r in results if r["match_status"] in
                       ("Not Found", "Mismatch / Duplicate"))
